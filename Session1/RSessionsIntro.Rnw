...
You may first need to install some packages on which it depends. It's
dependencies are: ncdf, nleqslv, rPython, ggplot2, ggthemes, grid,
reshape2. If any of those do not appear in your list of packages (displayed
...
directory select 'Ranadu-1.0-14-10-07.tar.gz' (or whatever the highest-version-number
is there).
