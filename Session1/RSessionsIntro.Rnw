...
To start your RStudio session, point a web browser to \href{http://tikal.eol.ucar.edu}{tikal.eol.ucar.edu}.
...
\end{enumerate}
\item On tikal, if that is where you will be running, copy this line into
...
the library and install it on another machine. 
\item If it's not already running, start RStudio; for tikal, you can use
the link above.
...
on tikal; in that case, knitr will be added to the available packages
when you follow the steps in the Appendix.
\end{enumerate}
...
standard on linux and Mac machines. These are both available on tikal.
...
For a preview of what is planned to follow this, see RSessionsDataFrame/zip
...
For use on tikal, a procedure suggested in the first section of this
...
dependencies are: ncdf, nleqslv, rPython (apparently not available
for Windows), ggplot2, ggthemes (apparently not available for Mac),
grid, reshape2. You will also want knitr if you don't already have
it. If any of those do not appear in your list of packages (displayed
...
directory select 'Ranadu-1.0-14-10-07-std.tar.gz' (or whatever the
highest-version-number is there). If installing on a machine other
than tikal, select the tar file appropriate for your architecture,
...
space. On my laptop it is '/home/cooperw/R/x86\_64-redhat-linux-gnu-library/3.1'.
